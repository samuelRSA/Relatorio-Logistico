...
import { revenueCostComboOption } from '@/features/executive/charts/executiveChartOptions';
...
export default function ExecutivePage() {
  ...
  const revenueCostOption = useMemo(() => revenueCostComboOption(comboData), [comboData]);
  ...
  const mapDataByUF = useMemo(() => buildBrazilMapData(allInvoices, mapFilters), [allInvoices, mapFilters]);
  const selectedUfData = useMemo(
    () => selectBrazilMapMetrics(mapDataByUF, activeUfs),
    [activeUfs, mapDataByUF],
  );

  const handleTransportDrilldown = useCallback(() => openDrilldown('transport-cost'), [openDrilldown]);
  const handleLogisticsResultDrilldown = useCallback(() => openDrilldown('logistics-result'), [openDrilldown]);
  ...
  return (
    <div className="space-y-5">
      <section className="grid grid-cols-6 gap-4">
        ...
        <KpiCard title="Resultado Logístico" value={formatCurrency(indicators.logisticsResult)} helper="Clique para rankings" accent="bg-signal-mint/20" icon={BadgeDollarSign} onClick={handleLogisticsResultDrilldown} />
      ...
      <section className="grid grid-cols-2 items-stretch gap-5">
        <ChartCard title="Receita x Custo Logístico" description="Barras de receita com linha de custo logístico">
          <EChart option={revenueCostOption} />
        </ChartCard>
